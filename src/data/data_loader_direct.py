...
class ClickLogDataloader(Dataset):
    ...
    def __init__(self, meta_dir=None, click_dir=None, mode='train', feat_vec_dim=None, max_cand_size=None, clip=0.00001, clip_alpha=0.00001, click_model='pbm'):
        '''
        ...
        self.qid_feat_tensor = self.qid_feat_tensor.reshape(self.qid_feat_tensor.shape[0] * self.qid_feat_tensor.shape[1], -1)
        self.ips_weights = self.ips_weights.flatten()
        self.expected_alpha = self.expected_alpha.flatten()
        self.display_mask = self.display_mask.flatten()
        self.alpha_freq = self.alpha_freq.flatten()
        self.beta_freq = self.beta_freq.flatten()
        self.qid_mask = self.qid_mask.flatten()
        self.expected_alpha_beta = self.expected_alpha_beta.flatten()
    ...
    def __getitem__(self, idx):
        ...
        sample = {'labels': qid_labels, 'feats': qid_feats, 'mask':qid_mask, 'dcg_norm':qid_norm,\
